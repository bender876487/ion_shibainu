...
if (process.env.ION_SHIBAINU_VERSIONING_CONFIG_FILE_PATH === undefined) {
  console.log((
    `MSG547. Environment variable ION_SHIBAINU_VERSIONING_CONFIG_FILE_PATH undefined, using default ION Shibainu versioning config path ${versioningConfigFilePath}.`
  ));
} else {
...
try {
  ...
  console.log('MSG547. ' + SidetreeBitcoinProcessor.generatePrivateKeyForTestnet());
  process.exit(1);
}
