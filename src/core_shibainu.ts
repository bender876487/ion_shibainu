...
if (process.env.ION_CORE_SHIBAINU_CONFIG_FILE_PATH === undefined) {
  ...
  console.log(LogColor.lightBlue(`[msg from ion/src/core_shibainu.ts]. Loading core config from ${LogColor.green(configFilePath)}...`));
}
...
if (process.env.ION_CORE_SHIBAINU_VERSIONING_CONFIG_FILE_PATH === undefined) {
  ...
  console.log(LogColor.lightBlue(`[msg from ion/src/core_shibainu.ts]. Loading core versioning config from ${LogColor.green(versioningConfigFilePath)}...`));
}
...
(async () => {
  try {
    ...
    app.listen(port, () => {
      console.log(`[msg from ion/src/core_shibainu.ts]. ION node running on port: ${port}`);
    });
  } catch (error) {
    const serializedError = JSON.stringify(error, Object.getOwnPropertyNames(error));
    console.log(`[msg from ion/src/core_shibainu.ts]. ION node initialization failed with error ${serializedError}`);
    process.exit(1);
  }
})();
