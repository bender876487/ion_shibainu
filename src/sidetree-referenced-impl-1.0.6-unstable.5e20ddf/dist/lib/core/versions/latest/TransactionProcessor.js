...
class TransactionProcessor {
    ...
    processTransaction(transaction) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (error instanceof SidetreeError_1.default) {
                    ...
                    if (error.code === ErrorCode_1.default.CasNotReachable ||
                        ...
                        Logger_1.default.info((`Invalid core file found for anchor string '${(transaction.anchorString)}', the entire batch is discarded. Error: ${(error.message)}`));
                        retryNeeded = false;
                    }
                }
                else {
                    Logger_1.default.error((`Unexpected error while fetching and downloading core files, MUST investigate and fix: ${error.message}`));
                    retryNeeded = true;
                }
            ...
            try {
                ...
                if (error instanceof SidetreeError_1.default) {
                    ...
                    if (error.code === ErrorCode_1.default.CasNotReachable ||
                        ...
                        // eslint-disable-next-line max-len
                        Logger_1.default.info((`Invalid provisional/chunk file found for anchor string '${(transaction.anchorString)}', the entire batch is discarded. Error: ${(error.message)}`));
                        retryNeeded = false;
                    }
                }
                else {
                    Logger_1.default.error((`Unexpected error while fetching and downloading provisional files, MUST investigate and fix: ${error.message}`));
                    retryNeeded = true;
                }
            }
            ...
            Logger_1.default.info((`Processed ${(operations.length)} operations. Retry needed: ${(retryNeeded)}`));
            const transactionProcessedCompletely = !retryNeeded;
            return transactionProcessedCompletely;
        });
    }
